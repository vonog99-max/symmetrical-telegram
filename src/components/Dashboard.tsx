...
export default function Dashboard({ onLogout }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'tokens' | 'actions' | 'raid' | 'settings' | 'admin' | 'rpc' | 'commands' | 'tos' | 'faq' | 'get_token' | 'rotator' | 'vc' | 'changelog' | 'revenge'>('tokens');
      ...
      {showChangelogModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <motion.div 
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="fantasy-card p-8 rounded-2xl max-w-2xl w-full relative overflow-hidden max-h-[80vh] flex flex-col"
          >
            <h2 className="text-2xl font-bold mb-4 text-emerald-400 flex items-center gap-2">
              <History className="w-6 h-6" /> What's New in {CHANGELOG[0].version}
            </h2>
              ...
              <div className="space-y-4">
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-mono text-zinc-500">{CHANGELOG[0].date}</span>
                  </div>
                ...
                <p className="text-xs text-zinc-500 italic text-center">
                  Click the "Changelog" tab in the sidebar to see previous updates.
                </p>
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-medium transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)]"
            >
              Got it!
            </button>
          </motion.div>
