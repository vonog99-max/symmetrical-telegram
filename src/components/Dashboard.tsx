...
import RevengeTab from './RevengeTab';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'tokens' | 'actions' | 'raid' | 'settings' | 'admin' | 'rpc' | 'commands' | 'tos' | 'faq' | 'get_token' | 'rotator' | 'vc' | 'changelog' | 'revenge' | 'hosting' | 'configs' | 'donation' | 'nitro_sniper'>('tokens');
        ...
        <div className="flex-1 flex flex-col overflow-hidden pt-16 md:pt-0">
          ...
          <main className="flex-1 overflow-y-auto p-8">
            ...
            >
              ...
              {activeTab === 'revenge' && (
                <RevengeTab token={loggedInToken} />
              )}
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden p-6">
                        ...
                        <div className="space-y-4 mb-8">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                  ...
                                  {/* Global Mass Boost */}
                                  <div className="space-y-2">
                                      <label className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Mass Boost (Server ID)</label>
                                      <div className="flex gap-2">
                                          <input 
                                              type="text" 
                                              placeholder="Server ID..." 
                                              value={massBoostInput}
                                              onChange={(e) => setMassBoostInput(e.target.value)}
                                              className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-300 focus:outline-none focus:border-pink-500/50"
                                          />
                                          <button 
                                              onClick={async () => {
                                                  if (massBoostInput) {
                                                      addLog(`Initiating Global Mass Boost for Server ID: ${massBoostInput}...`);
                                                      const res = await api.adminGlobalMassBoost(massBoostInput);
                                                      addLog(`Global mass boost finished. Processed for ${res.count} instances.`);
                                                      alert(`Success! ${res.message}`);
                                                      setMassBoostInput('');
                                                  }
                                              }} 
                                              className="bg-pink-600/20 hover:bg-pink-600/30 text-pink-400 px-3 py-2 rounded-lg text-sm border border-pink-500/20 transition-colors whitespace-nowrap"
                                          >
                                              Boost
                                          </button>
                                      </div>
                                            ...
                                            <td className="px-6 py-3 text-zinc-300 font-medium">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center text-xs text-zinc-500 overflow-hidden">
                                                        {s.avatar ? <img src={s.avatar} alt="" className="w-full h-full object-cover" /> : (s.username ? s.username[0].toUpperCase() : '?')}
                                                    </div>
