...
import { Upload, Trash2, Activity, Users, Settings, Skull, Mic, MessageSquare, UserPlus, LogOut, Flame, AlertTriangle, Type, Eraser, ShieldAlert, Gamepad2, Terminal, RefreshCw } from 'lucide-react';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'tokens' | 'actions' | 'raid' | 'settings' | 'admin' | 'rpc' | 'commands' | 'tos' | 'faq' | 'get_token' | 'rotator'>('tokens');
  ...
  // Status Rotator
  const [statusRotateText, setStatusRotateText] = useState('');
  ...
  const handleStatusRotate = async () => {
    if (!statusRotateText.trim()) return;
    setIsRotating(true);
    const statusList = statusRotateText.split('\n').map(s => s.trim()).filter(s => s);
    try {
      ...
      if (res.ok) {
        addLog(`Status rotation started with ${statusList.length} statuses`);
      } else {
        addLog('Failed to start status rotation');
      }
    } catch (e) {
              ...
              {activeTab === 'rotator' && (
                <div className="max-w-2xl mx-auto">
                  <div className="fantasy-card p-8 rounded-2xl">
                    ...
                    <div className="space-y-6">
                      <div>
                        <label className="block text-xs font-medium text-zinc-400 mb-2 uppercase tracking-wider">Statuses (One per line)</label>
                        <textarea 
                          placeholder="Playing Minecraft&#10;Streaming on Twitch&#10;Watching Netflix" 
                          className="fantasy-input w-full rounded-xl px-4 py-3 text-sm h-48 resize-none custom-scrollbar" 
                          value={statusRotateText}
                          onChange={(e) => setStatusRotateText(e.target.value)}
                        />
                        <p className="text-[10px] text-zinc-500 mt-2 italic">
                          Statuses will rotate every 10 seconds.
                        </p>
                      </div>
                      <button 
                ...
                <div className="space-y-6 max-w-3xl mx-auto">
                  <div className="fantasy-card p-8 rounded-2xl">
                    ...
                    <div className="space-y-6 text-sm text-zinc-300">
                      ...
                      <div>
                        <h4 className="font-bold text-white mb-1">Q: What is the "arlo" command?</h4>
                        <p className="text-zinc-400">A: It's an anti-nuke protection command. Use `.arlo &lt;guild_id&gt;` to toggle protection for a server. If someone tries to delete multiple channels/roles, it will strip their roles (if you have higher permissions).</p>
                      </div>
                      <div>
