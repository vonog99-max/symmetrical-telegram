...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    fetchUsage();
    const interval = setInterval(fetchUsage, 30000); // Check every 30s
            ...
            {usage && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={usage.isPremium ? "text-emerald-400 font-medium" : "text-amber-400 font-medium"}>
                    {usage.isPremium ? "Premium" : "Free Trial"}
                  </span>
                </div>
                {!usage.isPremium && (
                  <>
                    <div className="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min(100, (usage.totalUsageTime / usage.trialLimit) * 100)}%` }}
                      />
                    </div>
                    <p className="text-[10px] text-zinc-500 text-center">
                      {Math.max(0, Math.floor((usage.trialLimit - usage.totalUsageTime) / (1000 * 60)))} mins remaining
                    </p>
                    <button 
                      onClick={() => setActiveTab('settings')}
                      className="w-full py-1 text-[10px] bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded transition-colors"
                    >
                      Upgrade to Premium
                    </button>
                  </>
                ...
                {usage.isPremium && (
                  <p className="text-[10px] text-zinc-500 text-center">Unlimited access granted</p>
                )}
              </div>
            )}
          </div>
                ...
                <div className="max-w-2xl">
                  <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="pt-6 border-t border-zinc-800">
                        ...
                        <div className="flex items-start gap-6">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-zinc-500 mt-3">
                              This image will be used as the background for the .help command. (GIF SUPPORT ENABLED)
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-zinc-800">
                        <label className="block text-sm text-zinc-400 mb-4">Subscription & Usage</label>
                        <div className="bg-zinc-950 rounded-xl p-6 border border-zinc-800">
                          <div className="flex items-center justify-between mb-6">
                            <div>
                              <p className="text-white font-medium">
                                {usage?.isPremium ? 'Premium Plan' : 'Free Trial'}
                              </p>
                              <p className="text-xs text-zinc-500 mt-1">
                                {usage?.isPremium ? 'Unlimited selfbot usage enabled' : '7 hours of usage per 24-hour cycle'}
                              </p>
                            </div>
                            <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${usage?.isPremium ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>
                              {usage?.isPremium ? 'Active' : 'Trial'}
                            </div>
                          </div>
                            ...
                            <div className="space-y-4">
                              <div>
                                <div className="flex justify-between text-xs mb-2">
                                  ...
                                  <span className="text-zinc-300">
                                    {usage ? Math.floor(usage.totalUsageTime / (1000 * 60 * 60)) : 0}h / 7h
                                  </span>
                                </div>
                                <div className="w-full bg-zinc-900 h-2 rounded-full overflow-hidden border border-zinc-800">
                                  <div 
                                    className="bg-gradient-to-r from-amber-600 to-amber-400 h-full transition-all duration-1000" 
                                    style={{ width: `${usage ? Math.min(100, (usage.totalUsageTime / usage.trialLimit) * 100) : 0}%` }}
                                  />
                                </div>
                              </div>
                              ...
                              <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-4">
                                <p className="text-xs text-amber-200/80 leading-relaxed">
                                  <span className="font-bold text-amber-400">Note:</span> Once your 7-hour trial expires, all active sessions will be stopped and you will be blocked from logging in for 24 hours.
                                </p>
                              </div>

                              <div className="pt-2">
                                <p className="text-sm text-white mb-2">Want unlimited access?</p>
                                <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800 flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <p className="text-xs text-zinc-400">Contact Admin</p>
                                      <p className="text-sm text-white font-mono">DM HADE (@881x.)</p>
                                    </div>
                                  </div>
                                  <button className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-medium rounded-lg transition-colors">
                                    Get Premium
                                  </button>
                                </div>
                              </div>
                          ...
                          {usage?.isPremium && (
                            <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-4 flex items-center gap-3">
                              <Activity className="w-5 h-5 text-emerald-500" />
                              <p className="text-xs text-emerald-200/80">
                                You have full access to all features. Thank you for supporting the project!
                              </p>
                            </div>
                          )}
                        </div>
...
/>}
              ...
              {activeTab === 'admin' && isAdmin && (
                <div className="space-y-6">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                        <h3 className="text-sm font-medium text-zinc-300 mb-4">Manage Premium Status</h3>
                        <div className="flex gap-4">
                            <input 
                                type="text" 
                                placeholder="User ID" 
                                className="bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-sm flex-1"
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    const res = await fetch('/api/admin/set-premium', {
                                        ...
                                        body: JSON.stringify({ targetUserId: adminTargetUserId, isPremium: true })
                                    });
                                    if (res.ok) {
                                        setLogs(prev => [`[${new Date().toLocaleTimeString()}] Admin: Granted premium to ${adminTargetUserId}`, ...prev]);
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    const res = await fetch('/api/admin/set-premium', {
                                        ...
                                        body: JSON.stringify({ targetUserId: adminTargetUserId, isPremium: false })
                                    });
                                    if (res.ok) {
                                        setLogs(prev => [`[${new Date().toLocaleTimeString()}] Admin: Revoked premium from ${adminTargetUserId}`, ...prev]);
                                        setAdminTargetUserId('');
                                    }
                                }}
