...
export default function RevengeTab({ token }: { token: string }) {
  ...
  const handleGhostPing = async () => {
    const channelId = prompt('Enter Channel ID:');
    const userId = prompt('Enter User ID:');
    if (!channelId || !userId) return;
    setIsLoading(true);
    try {
      await api.ghostPing(token, channelId, userId);
      setStatus(`GHOST PING: Pinged ${userId} in ${channelId}`);
    } catch (err) {
      setStatus(`Error: ${err}`);
    } finally {
      setIsLoading(false);
  ...
  const handleMassSpam = async () => {
    const channelId = prompt('Enter Channel ID:');
    const message = prompt('Enter Message:');
    const count = parseInt(prompt('Enter Count:') || '0');
    if (!channelId || !message || count <= 0) return;
    setIsLoading(true);
    try {
      await api.spam(channelId, message, count);
      setStatus(`MASS SPAM: Sent ${count} messages to ${channelId}`);
    } catch (err) {
      setStatus(`Error: ${err}`);
    } finally {
      setIsLoading(false);
    ...
    <div className="space-y-6">
      <div className="bg-black/40 border border-white/10 rounded-xl p-6">
        ...
        <div className="space-y-4">
          <div className="flex gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-black/20 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-300 focus:outline-none focus:border-red-500/50"
              />
            </div>
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={isLoading || !targetId}
                className="px-4 py-2 bg-black/20 border border-white/10 hover:bg-white/5 disabled:opacity-50 text-zinc-300 rounded-lg text-sm font-medium transition-all flex items-center gap-2"
          ...
          {status && (
            <div className="p-3 bg-black/20 border border-white/10 rounded-lg flex items-center gap-2">
      ...
      <div className="bg-black/40 border border-white/10 rounded-xl p-6">
        ...
        <div className="space-y-2">
          {termedUsers.length === 0 ? (
            ...
            termedUsers.map((id) => (
              <div key={id} className="flex items-center justify-between p-3 bg-black/20 border border-white/10 rounded-lg group">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-black/40 border border-white/10 rounded-xl p-6">
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full py-2 bg-black/20 border border-white/10 hover:bg-white/5 text-zinc-300 rounded-lg text-sm transition-all"
          >
