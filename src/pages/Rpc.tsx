...
export default function Rpc() {
    ...
    const handleUpdate = async () => {
        ...
        if (typeof window !== 'undefined' && window.location.hostname.includes('netlify.app')) {
            setTimeout(() => setStatus('RPC Updated Successfully! (Mock)'), 500);
            return;
        }
        try {
            const res = await fetch('/api/rpc/update', {
                method: 'POST',
    ...
    const handleClear = async () => {
        if (typeof window !== 'undefined' && window.location.hostname.includes('netlify.app')) {
            setStatus('RPC Cleared (Mock)');
            return;
        }
        try {
            await fetch('/api/rpc/clear', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'large' | 'small') => {
        ...
        if (typeof window !== 'undefined' && window.location.hostname.includes('netlify.app')) {
            setStatus('Uploading image... (Mock)');
            setTimeout(() => {
                setConfig(prev => ({
                    ...prev,
                    [target === 'large' ? 'largeImageKey' : 'smallImageKey']: 'https://picsum.photos/200'
                }));
                setStatus('Image uploaded and URL set! (Mock)');
            }, 1000);
            return;
        }
        ...
        try {
            const res = await fetch('/api/rpc/upload-image', {
                method: 'POST',
                body: formData
            });
            const data = await res.json();
