...
export default function Rpc({ configs, setConfigs, selectedIndex, setSelectedIndex, hoursElapsed, setHoursElapsed }: Omit<RpcProps, 'token' | 'setToken'>) {
    ...
    const [showCrLogin, setShowCrLogin] = useState(false);
    const [crEmail, setCrEmail] = useState('');
    const [crPassword, setCrPassword] = useState('');
    const [crLoggingIn, setCrLoggingIn] = useState(false);
    ...
    React.useEffect(() => {
        const handleMessage = async (event: MessageEvent) => {
            if (event.data?.type === 'OAUTH_AUTH_SUCCESS' && event.data.user) {
                ...
                // Fetch saved CR token
                try {
                    const res = await fetch(`${API_BASE}/api/user/get-cr-token/${user.id}`);
                    const data = await res.json();
                    if (data.crToken) {
                        setConfig(prev => ({ ...prev, crToken: data.crToken }));
                        setStatus(`Welcome back ${user.username}! Crunchyroll token restored.`);
                    }
                } catch (e) {
                    console.error('Failed to restore CR token:', e);
                }
            }
        };
    ...
    const handleDiscordLogin = async () => {
        try {
            ...
            const res = await fetch(`${API_BASE}/api/auth/discord/url?redirect_uri=${redirectUri}`);
            const { url } = await res.json();
            window.open(url, 'discord_auth', 'width=600,height=700');
        } catch (e) {
            setStatus('Failed to get Discord login URL');
        }
    };

    const handleCrLogin = async () => {
        if (!crEmail || !crPassword) {
        ...
        setCrLoggingIn(true);
        try {
            const res = await fetch(`${API_BASE}/api/crunchyroll/login`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: crEmail, password: crPassword })
            });

            const data = await res.json();
            if (res.ok && data.access_token) {
                setConfig(prev => ({ ...prev, crToken: data.access_token }));
                setStatus('Successfully retrieved Bearer token!');
                setShowCrLogin(false);
                setCrPassword(''); // Clear password

                // Save to Supabase if logged in with Discord
                if (discordUser) {
                    try {
                        await fetch(`${API_BASE}/api/user/save-cr-token`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ discordId: discordUser.id, crToken: data.access_token })
                        });
                    } catch (e) {
                        console.error('Failed to save CR token to cloud:', e);
                    }
                }
            } else {
            ...
            setStatus('Error connecting to login service');
        } finally {
            setCrLoggingIn(false);
        }
    };
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'large' | 'small') => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                ...
                // Determine aspect ratio based on config
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto text-zinc-100">
            ...
            <div className="bg-black/20 border border-white/10 rounded-xl p-6 mb-8">
                    <div className="mb-6 flex gap-2 overflow-x-auto pb-2">
                        ...
                        <button
                            onClick={() => {
                                ...
                                setStatus('Crunchyroll Preset Applied!');
                            }}
                            className="px-4 py-2 rounded-lg text-sm font-medium bg-orange-600/20 border border-orange-500/30 text-orange-400 hover:bg-orange-500/30"
                        >
                            Crunchyroll Preset
                        </button>
                        <button
                            onClick={() => {
                                setConfig({
                                    ...
                                    type: 'PLAYING',
                                    url: '',
                                    applicationId: '1015931589851959326', // Samsung Galaxy (Portrait)
                                    startTimestamp: Date.now().toString()
                                });
                ...
                {status && (
                    <div className="mt-4 p-3 bg-black/20 border border-white/10 rounded text-sm text-center text-zinc-400">
                        {status}
                    </div>
                )}
            </div>
