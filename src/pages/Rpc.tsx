...
export default function Rpc({ token, setToken, configs, setConfigs, selectedIndex, setSelectedIndex, hoursElapsed, setHoursElapsed }: RpcProps) {
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto text-zinc-100">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 mb-8">
                <div className="mb-6 flex gap-2 overflow-x-auto pb-2">
                    {configs.map((_, i) => (
                        <button
                            ...
                            onClick={() => {
                                setSelectedIndex(i);
                                // We need to wait for the state to update, but handleUpdate uses the current state.
                                // A simple way is to use a useEffect, but let's just call it here with a slight delay or just call it directly.
                                // Actually, handleUpdate uses the current configs and selectedIndex state.
                                // If we call it here, it will use the OLD selectedIndex.
                                // So we should use a useEffect.
                            }}
                ...
                <div className="mb-6">
                    <label className="block text-xs font-medium text-zinc-400 mb-2 uppercase tracking-wider">Target Token (Optional)</label>
                    <input 
                        type="text" 
                        value={token}
                        onChange={e => setToken(e.target.value)}
                        placeholder="Paste your token here (or leave blank if already active)..."
