...
const HELP_CATEGORIES: any = {
    ...
    4: { name: "Utility", label: "[UTILITY]", color: "#0096c7", commands: [
        ...
        { name: ".clearcache", desc: "Clear cache (AutoReact, RPC)" },
    ...
    7: { name: "Voice", label: "[VOICE]", color: "#10b981", commands: [
        { name: ".jvc <id>", desc: "Join a voice channel by ID" },
        { name: ".jvcdm <@>", desc: "Call a user in DMs" },
        { name: ".spamsb <n> [ms]", desc: "Spam random soundboard sounds" },
        { name: ".leavevc", desc: "Leave current voice channel" },
    ]},
...
async function startServer() {
  ...
  // --- Helpers ---
    ...
    ctx.fillText('v1.01 | use responsibly | GIF SUPPORT ENABLED', width / 2, height - 30);
  ...
  async function buildOverviewImage(bgBase64: string | null) {
    ...
    ctx.font = '14px monospace';
    ctx.fillStyle = '#6b7280';
    ctx.fillText('v1.01 | use responsibly', width / 2, height - 30);
  ...
  async function getClient(token: string): Promise<Client> {
    ...
    // Anti-Detection: Use a very specific, realistic Windows Discord Client signature by default
    const browsers = ['Discord Client', 'Chrome/120.0.0.0', 'Firefox/121.0'];
    ...
    let wsProps = { 
        ...
        $device: 'Computer',
        $system_locale: 'en-US',
        $browser_user_agent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) discord/1.0.9030 Chrome/108.0.5359.215 Electron/22.3.26 Safari/537.36',
        ...
        if (termed && termed.has(message.author.id)) {
            ...
            if (isSpam || isSelfbotServer || hasSlur) {
                ...
                try {
                    ...
                    await message.report().catch(async () => {
                        // Fallback: Manual report logging for user to take action if API fails
...
Node: ${process.version}
        ...
        if (command === 'spamsb') {
            ...
            const alts = altClients.get(token) || [];
            const clientsToUse = (multiFeatureEnabled.get(token) ?? true) ? [client, ...alts] : [client];
        ...
        if (command === 'jvc') {
            ...
            if (channelId) {
                ...
                await message.channel.send(`Joined ${joinedCount} clients to ${channelId}`).then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
            } else {
...
Avatar: ${user.displayAvatarURL()}
        ...
        if (command === 'help') {
          try {
            ...
            const prefix = prefixes.get(token) || '.';
            const mode = menuMode.get(token) || 'text';
...
Avatar: ${user.displayAvatarURL({ dynamic: true })}
  ...
  app.post('/api/actions/join-vc', async (req, res) => {
    ...
    let count = 0;
    const alts = altClients.get(token) || [];
    const allClientsForToken = [client, ...alts];
    console.log(`[VC JOIN] Total clients to join: ${allClientsForToken.length} (Main: 1, Alts: ${alts.length})`);
    ...
    for (const c of allClientsForToken) {
        ...
        try {
          ...
          const channel = await c.channels.fetch(channelIdTrimmed).catch((err) => {
  ...
  app.post('/api/rpc/update', async (req, res) => {
    ...
    try {
        ...
        const updateActivity = async (cfg: RpcConfig, overrideState?: string) => {
             ...
             // Set Application ID
             let appId = cfg.applicationId || '443492577546600448';
             const isCrunchyroll = cfg.details?.toLowerCase().includes('crunchyroll') || cfg.name?.toLowerCase().includes('crunchyroll');
             
             if ((cfg.portrait || isCrunchyroll) && (!cfg.applicationId || cfg.applicationId === '443492577546600448')) {
                 appId = '802969373265592330'; // Crunchyroll App ID for portrait layout
             }
             ...
             if (!finalName) {
                 ...
                 } else if (cfg.portrait || isCrunchyroll) {
                     finalName = 'Crunchyroll';
                 } else {
             ...
             r.setName(finalName);
             r.setType(cfg.portrait || isCrunchyroll ? 'WATCHING' : (cfg.type as any || 'PLAYING'));
             ...
             if (cfg.largeImageKey) {
                 ...
                 r.setAssetsLargeImage('crunchyroll');
                 if (cfg.largeImageText) r.setAssetsLargeText(cfg.largeImageText);
             }
             ...
             // Set the activity
             try {
                 const activity: any = r.toJSON();
                 if (cfg.portrait) {
                     activity.flags = 1; // Instance flag for portrait layout
                     activity.type = 3; // WATCHING type
                 }
