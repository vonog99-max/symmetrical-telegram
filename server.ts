  ...
  async function getClient(token: string): Promise<Client> {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('voiceStateUpdate', (oldState, newState) => {
          ...
          const autoReconnect = autoReconnectConfigs.get(token);
...
Node: ${process.version}
        ...
        if (command === 'autoquest') {
            ...
            try {
                ...
                // Filter for enrolled but not completed
                const active = quests.filter((x: any) => x.user_status?.enrolled_at && !x.user_status?.completed_at);
                ...
                await message.channel.send(`> ⏳ Found ${active.length} active quests. Starting automation...`).catch(() => {});
                ...
                for (const quest of active) {
                    ...
                    addLog(token, `Starting Quest: ${quest.config.messages.quest_name} (${taskName})`);
                    ...
                    if (taskName === 'WATCH_VIDEO' || taskName === 'WATCH_VIDEO_ON_MOBILE') {
                        ...
                        addLog(token, `Completed Quest: ${quest.config.messages.quest_name}`);
                    } else if (taskName === 'PLAY_ON_DESKTOP' || taskName === 'PLAY_ACTIVITY' || taskName === 'STREAM_ON_DESKTOP') {
                        const appId = quest.config.application.id;
                        const r = new RichPresence(client)
                            .setApplicationId(appId)
                            .setType('PLAYING')
                            .setName(quest.config.application.name);
                        
                        client.user?.setPresence({ activities: [r] });
                        
                        let completed = false;
                        let attempts = 0;
                        while (!completed && attempts < 20) { // Max 10 mins
                            attempts++;
                            ...
                            if (hbRes.ok) {
                                ...
                                if (hbData.user_status?.completed_at) completed = true;
                            }
                ...
                await message.channel.send(`> ✅ All possible quests completed!`).catch(() => {});
            } catch (e) {
    ...
    try {
        ...
        const updateActivity = async (cfg: RpcConfig, overrideState?: string) => {
             ...
             if (cfg.button1Label && cfg.button1Url) {
                 try { r.addButton(cfg.button1Label, sanitizeUrl(cfg.button1Url)); } catch(e) { console.error('Button 1 error:', e); }
             }
             if (cfg.button2Label && cfg.button2Url) {
                 try { r.addButton(cfg.button2Label, sanitizeUrl(cfg.button2Url)); } catch(e) { console.error('Button 2 error:', e); }
             }
