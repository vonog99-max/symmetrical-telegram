              ...
              if (isVoice) {
                  ...
                  if (!connection) {
                      ...
                      if (client.voice && typeof client.voice.joinChannel === 'function') {
                          // @ts-ignore
                          connection = await client.voice.joinChannel(channel, { selfVideo: state.enabled, selfDeaf: true }).catch(err => {
                              console.error(`[STREAM-MONITOR] Join error for ${token}:`, err.message);
                              return null;
                          });
                      } else {
                          // @ts-ignore
                          connection = await client.voice?.join(channel, { selfVideo: state.enabled, selfDeaf: true }).catch(err => {
                              console.error(`[STREAM-MONITOR] Join error for ${token}:`, err.message);
                              return null;
                          });
                      }
  ...
  app.post('/api/actions/join-vc', async (req, res) => {
    ...
    for (const c of allClientsForToken) {
        ...
        try {
          ...
          console.log(`[VC JOIN] Client ${tag} attempting to fetch channel ${channelId}`);
          const channel = await c.channels.fetch(channelId).catch((err) => {
              console.error(`[VC JOIN] Fetch error for ${tag}:`, err.message);
              return null;
          });
          
          if (!channel) {
              addLog(token, `Channel ${channelId} not found for client ${tag}`);
              console.log(`[VC JOIN] Channel ${channelId} not found for ${tag}`);
              continue;
          }
          ...
          if (isVoice) {
             ...
             const joinOptions = { selfDeaf: true, selfMute: true, timeout: 30000 };
  ...
  app.post('/api/rpc/update', async (req, res) => {
    ...
    try {
        ...
        const updateActivity = async (cfg: RpcConfig, overrideState?: string) => {
             ...
             if (cfg.portrait && !cfg.applicationId) {
                 appId = '802969373265592330'; // Working App ID for portrait layout
             }
             r.setApplicationId(appId);

             // Basic Info
             r.setName(cfg.name || '');
             r.setType(cfg.portrait ? 'WATCHING' : (cfg.type as any || 'WATCHING'));
             if (cfg.details) r.setDetails(cfg.details);
             if (cfg.state) r.setState(overrideState || cfg.state || '');
             ...
             // Streaming URL
             if (cfg.type === 'STREAMING' && cfg.url) {
                 r.setURL(cfg.url);
             }
             
             // Custom Buttons
             if (cfg.button1Label && cfg.button1Url) {
                 r.addButton(cfg.button1Label, cfg.button1Url);
             }
             if (cfg.button2Label && cfg.button2Url) {
                 r.addButton(cfg.button2Label, cfg.button2Url);
             }
