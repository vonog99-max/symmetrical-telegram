...
async function startServer() {
  ...
  const getClient = async (token: string): Promise<Client> => {
    ...
    const client = new Client({
      // @ts-ignore
      ws: { properties: { $os: 'Windows', $browser: 'Discord Client', $device: '' } },
      waitGuildTimeout: 0
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('debug', (info) => {
        ...
        if (info.includes('[USER_REQUIRED_ACTION]')) {
          clearTimeout(timeout);
          activeClients.delete(token);
          client.destroy();
          reject(new Error(`Account requires verification: ${info}`));
        }
      });
      ...
      client.on('shardDisconnect', (event, id) => {
        ...
        activeClients.delete(token);
        client.destroy();
        reject(new Error(`Disconnected from Discord (Code: ${event.code}). Token may be invalid or locked.`));
      });
      ...
      client.on('invalidated', () => {
        ...
        activeClients.delete(token);
        client.destroy();
        reject(new Error('Session invalidated (possibly invalid token).'));
      });
