  ...
  app.post('/api/actions/status-rotate', async (req, res) => {
    ...
    for (const c of allClientsForToken) {
        try {
          ...
          let index = 0;
          const setCustomStatus = (text: string) => {
            // @ts-ignore
            c.user?.setPresence({
                activities: [{
                    name: "Custom Status",
          ...
          setCustomStatus(statusList[index]);
  ...
  app.post('/api/rpc/update', async (req, res) => {
    ...
    try {
        ...
        const updateActivity = async (overrideState?: string) => {
             ...
             // Set Application ID (defaults to Crunchyroll official ID)
             const appId = config.applicationId || '443492577546600448';
             r.setApplicationId(appId);

             // Basic Info
             r.setName(config.name || 'Crunchyroll');
             r.setType(config.type || 'WATCHING'); // Default to WATCHING as it's most common for Crunchyroll
             ...
             if (config.portrait) {
                 ...
                 // Force timestamps if not set (required for portrait layout)
                 if (!config.startTimestamp) {
                     const now = Date.now();
                     r.setStartTimestamp(now);
                     // Set massive duration end timestamp
                     r.setEndTimestamp(now + (41666 * 60 * 60 * 1000));
                 }

                 // Add "Watch" button if not present in config
                 // Note: selfbot-v13 RichPresence uses addButton(label, url)
                 // We check if buttons are already added via config (if supported later)
                 // For now, we just add the Watch button
                 r.addButton('Watch', config.url || 'https://www.crunchyroll.com/');
             }
