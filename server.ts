...
Avatar: ${user.displayAvatarURL({ dynamic: true })}
  ...
  app.post('/api/actions/join-vc', async (req, res) => {
    ...
    for (const c of allClientsForToken) {
        try {
          console.log(`[VC JOIN] Client ${c.user?.tag} attempting to fetch channel ${channelId}`);
          const channel = await c.channels.fetch(channelId).catch((err) => {
              console.error(`[VC JOIN] Fetch error for ${c.user?.tag}:`, err.message);
              return null;
          });
          
          if (!channel) {
              addLog(token, `Channel ${channelId} not found for client ${c.user?.tag}`);
              console.log(`[VC JOIN] Channel ${channelId} not found for ${c.user?.tag}`);
              continue;
          }

          console.log(`[VC JOIN] Found channel ${channel.name} (Type: ${channel.type}) for ${c.user?.tag}`);

          // Check if it's a voice channel (Type 2 is GUILD_VOICE, Type 13 is GUILD_STAGE_VOICE)
          ...
          if (isVoice) {
             addLog(token, `Attempting to join VC: ${channel.name} (${c.user?.tag})...`);
             // @ts-ignore
             console.log(`[VC JOIN] Voice manager properties for ${c.user?.tag}:`, Object.keys(c.voice || {}));
             ...
             try {
                 ...
                 if (c.voice && typeof c.voice.joinChannel === 'function') {
                     console.log(`[VC JOIN] Calling c.voice.joinChannel for ${c.user?.tag}`);
                     // @ts-ignore
                     connection = await c.voice.joinChannel(channel, joinOptions);
                 } else if (typeof channel.join === 'function') {
                     console.log(`[VC JOIN] Calling channel.join for ${c.user?.tag}`);
                     // @ts-ignore
                     connection = await channel.join(joinOptions);
                 }

                 if (connection) {
                     addLog(token, `Successfully joined VC: ${channel.name} (${c.user?.tag})`);
                     console.log(`[VC JOIN] Success for ${c.user?.tag}`);
                     count++;
                 } else {
                     addLog(token, `Join method not found or failed for ${c.user?.tag}`);
                 }
             } catch (joinErr) {
                 if (joinErr.message.includes('TIMEOUT')) {
                     addLog(token, `VC Join Timeout for ${c.user?.tag}. Retrying once...`);
                     // Retry once after a short delay
                     await new Promise(r => setTimeout(r, 2000));
                     // @ts-ignore
                     connection = await (c.voice?.joinChannel?.(channel, joinOptions) || channel.join?.(joinOptions));
                     if (connection) {
                         addLog(token, `Successfully joined VC on retry: ${channel.name} (${c.user?.tag})`);
                         count++;
                     }
                 } else {
                     throw joinErr;
          ...
          addLog(token, `Failed to join VC for ${c.user?.tag}: ${e.message}`);
          console.error(`[VC JOIN] Critical Error [${c.user?.tag}]:`, e);
        }
    }
    if (count === 0) {
