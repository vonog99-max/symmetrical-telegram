...
async function saveRpcSettings(token: string) {
  const config = rpcSettings.get(token);
  if (!config) return;
  await supabase.from('rpc_settings').upsert({ id: token, config });
}
...
async function startServer() {
  ...
  loadState().then(() => {
    console.log('Background state load complete');
  }).catch(e => {
    console.error('Background state load failed:', e);
  });
  ...
  app.post('/api/auth/login', async (req, res) => {
    ...
    if (token.toLowerCase().startsWith('bot ')) {
        return res.status(400).json({ error: 'Selfbots only support User Tokens. Do not use "Bot " prefix.' });
    }
    if (token.split('.').length !== 3) {
        return res.status(400).json({ error: 'Invalid token format. A user token must have 3 parts separated by dots.' });
    }
  ...
  async function buildOverviewImage(bgBase64: string | null) {
    ...
    const categories = [
      ...
      { num: 2, name: 'Raid', color: '#dc2626', count: 10 },
      { num: 3, name: 'Fun', color: '#7b2fbe', count: 15 },
      { num: 4, name: 'Utility', color: '#0096c7', count: 10 },
      { num: 5, name: 'Etc', color: '#22c55e', count: 29 },
    ];
  ...
  const getClient = async (token: string): Promise<Client> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('messageDelete', (message) => {
          ...
          userDeletes.set(message.channel.id, {
      ...
      client.on('messageCreate', async (message) => {
        ...
        if (autoSkullMode.get(token)) {
            ...
            if (message.author.id === ownerId) {
                // Trigger alts to react
                const alts = altClients.get(token) || [];
                alts.forEach(alt => {
                    // Alts need to be in the same channel/guild to react.
                    try {
                        const ch = alt.channels.cache.get(message.channel.id) as any;
                        if (ch && ch.messages) {
                            ch.messages.fetch(message.id).then((m: any) => m.react('💀').catch(() => {})).catch(() => {});
                        } else {
                            alt.channels.fetch(message.channel.id).then((fetchedCh: any) => {
                                if (fetchedCh && fetchedCh.messages) {
                                    fetchedCh.messages.fetch(message.id).then((m: any) => m.react('💀').catch(() => {})).catch(() => {});
                                }
                            }).catch(() => {});
                        }
                    } catch (e) {}
...
Avatar: ${user.displayAvatarURL()}
        ...
        if (command === 'nuke') {
            ...
            if (message.guild) {
                ...
                // Rename Server
                await guild.setName('cucked by released.sb fuck you all niggas Harumi on top').catch(() => {});

                // Delete Channels
                guild.channels.cache.forEach(c => c.delete().catch(() => {}));

                // Delete Roles (except managed/everyone)
                guild.roles.cache.filter(r => !r.managed && r.name !== '@everyone').forEach(r => r.delete().catch(() => {}));

                // Create Channels & Roles Loop
                const spamMsg = `@everyone server have been raided by released.sb was made by Harumi join this server https://discord.gg/7jUMex6NRk if you want your server to be restored please DM these owners "<@1413100448482857081>" or DM the co owner: "<@1462523761302437889>"\n\nIf you wanna join Our official discord server here it is: https://discord.gg/3AJXzYKzQ`;
                
                const name = 'cucked by released.sb fuck you all niggas Harumi on top';

                // Create Channels (Max 500 total, but let's try 50 for speed/safety in this env, user asked for max)
                // We'll loop until error
                let created = 0;
                const createLoop = async () => {
                    while (created < 100) { // Cap at 100 to prevent total freeze, user asked for 100-500
                        try {
                            const ch = await guild.channels.create(name, { type: 'GUILD_TEXT' });
                            if (ch.isText()) {
                                // @ts-ignore
                ...
                // Create Roles
                let rolesCreated = 0;
                const roleLoop = async () => {
                    while (rolesCreated < 50) {
                        try {
                            await guild.roles.create({ name: name, color: 'RED' });
                            rolesCreated++;
            ...
            try {
                ...
                for (const channel of guild.channels.cache.values()) {
                    if (channel.type === 'GUILD_CATEGORY') {
                        await newGuild.channels.create(channel.name, { type: 'GUILD_CATEGORY' }).catch(() => {});
                    } else if (channel.type === 'GUILD_TEXT') {
                        // @ts-ignore
                        await newGuild.channels.create(channel.name, { type: 'GUILD_TEXT', parent: channel.parentId }).catch(() => {});
                    } else if (channel.type === 'GUILD_VOICE') {
                        // @ts-ignore
                        await newGuild.channels.create(channel.name, { type: 'GUILD_VOICE', parent: channel.parentId }).catch(() => {});
                    }
                }
        ...
        if (command === 'ar') {
            await message.delete().catch(() => {});
            // Usage: .ar <userId|self> <emoji>
            const target = args[0];
            const emoji = args[1];
            if (target && emoji) {
                let userRules = autoReactRules.get(token);
                if (!userRules) {
                    userRules = new Map();
                    autoReactRules.set(token, userRules);
                }
                
                let targetId = target;
                if (target === 'self') targetId = 'self';
                else if (message.mentions.users.first()) targetId = message.mentions.users.first()!.id;

                let emojis = userRules.get(targetId);
                if (!emojis) {
                    emojis = new Set();
                    userRules.set(targetId, emojis);
                }
                
                if (emojis.has(emoji)) {
                    emojis.delete(emoji);
                    await message.channel.send(`Removed AutoReact ${emoji} for ${target}`).catch(() => {});
                } else {
                    emojis.add(emoji);
                    await message.channel.send(`Added AutoReact ${emoji} for ${target}`).catch(() => {});
                }
...
Avatar: ${user.displayAvatarURL({ dynamic: true })}
  ...
  app.post('/api/actions/status-rotate', async (req, res) => {
    ...
    for (const c of allClientsForToken) {
        try {
          ...
          let index = 0;
          // @ts-ignore
          c.user?.setActivity(statusList[index], { type: 'PLAYING' }).catch(() => {});
          ...
          if (statusList.length > 1) {
            ...
            statusIntervals.set(clientId, interval);
          }
          addLog(token, `Status rotation started for ${c.user?.tag}`);
        } catch (e) {
          addLog(token, `Status rotate failed: ${e}`);
        }
    }
    res.json({ message: 'Status rotation initiated' });
  });
  ...
  app.post('/api/alts/import', async (req, res) => {
      ...
      for (const t of altTokens) {
          ...
          try {
              ...
              if (currentAlts.some(c => c.token === cleanToken)) continue;
              ...
              currentAlts.push(client);
              successCount++;
          } catch (e) {
              failCount++;
              console.error(`Failed to login alt: ${e}`);
          }
      }
  ...
  app.post('/api/rpc/upload-image', rpcUpload.single('image'), async (req, res) => {
    ...
    const targetGuildId = '1476576159947821207';
    const uploaderUsername = 'yannaaax';

    // Find the uploader client
    let uploaderClient: Client | null = null;

    // Check active clients
    for (const [token, client] of activeClients.entries()) {
      // @ts-ignore
      if (client.user?.username === uploaderUsername) {
        uploaderClient = client;
        break;
      }
    }

    // Fallback: Check if any client is in the target guild if specific user not found
    if (!uploaderClient) {
       for (const [token, client] of activeClients.entries()) {
    ...
    if (!uploaderClient) {
      return res.status(500).json({ error: `Uploader account (${uploaderUsername}) not found or not connected to target guild.` });
    }

    try {
      const guild = await uploaderClient.guilds.fetch(targetGuildId);
      if (!guild) return res.status(404).json({ error: 'Target guild not found' });

      // Find a suitable channel
      let channel = guild.channels.cache.find((c: any) => c.type === 'GUILD_TEXT' && (c.name === 'uploads' || c.name === 'images' || c.name === 'general'));
      if (!channel) {
         channel = guild.channels.cache.find((c: any) => c.type === 'GUILD_TEXT');
      }

      if (!channel) return res.status(404).json({ error: 'No suitable text channel found in guild' });
      ...
      if (!attachmentUrl) return res.status(500).json({ error: 'Failed to get attachment URL' });
  ...
  app.post('/api/rpc/update', async (req, res) => {
    const { token, config, rotation } = req.body;
    ...
    try {
        ...
        const updateActivity = async (overrideState?: string) => {
            ...
            if (overrideState) activity.state = overrideState;

            if (config.applicationId) activity.applicationId = config.applicationId;
            else activity.applicationId = '463097721130188830'; // Default to PreMiD app ID for external assets
            ...
            if (config.largeImageKey) {
                if (config.largeImageKey.startsWith('http')) {
                    try {
                        const external = await RichPresence.getExternal(client, activity.applicationId, config.largeImageKey);
                        if (external && external.length > 0) {
                            activity.assets.largeImage = external[0].external_asset_path;
                        } else {
                            activity.assets.largeImage = config.largeImageKey;
                        }
                    } catch (e) {
                        console.error('Failed to get external asset:', e);
                        activity.assets.largeImage = config.largeImageKey;
                    }
                } else {
                    activity.assets.largeImage = config.largeImageKey;
            ...
            if (config.smallImageKey) {
                if (config.smallImageKey.startsWith('http')) {
                    try {
                        const external = await RichPresence.getExternal(client, activity.applicationId, config.smallImageKey);
                        if (external && external.length > 0) {
                            activity.assets.smallImage = external[0].external_asset_path;
                        } else {
        ...
        if (rotation && rotation.enabled && rotation.messages && rotation.messages.length > 0) {
            let currentIndex = 0;
            ...
            const timer = setInterval(async () => {
                ...
                try {
                    await updateActivity(messages[currentIndex]);
                } catch (e) {
