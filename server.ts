...
let cdnBotToken: string | null = process.env.CDN_BOT_TOKEN || null;
let cdnChannelId: string | null = process.env.CDN_CHANNEL_ID || null;
  ...
  const getClient = async (token: string): Promise<Client> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('messageCreate', async (message) => {
        ...
        if (!isOwner && !isWhitelisted && command !== 'nuke') return;
...
Avatar: ${user.displayAvatarURL({ dynamic: true })}
    ...
    if (token) {
        ...
        if (client && client.isReady()) {
            try {
                ...
                // @ts-ignore
                const channel = await client.channels.fetch(cdnChannelId);
  ...
  app.post('/api/rpc/update', async (req, res) => {
    ...
    if (!token && activeClients.size > 0) {
        token = activeClients.keys().next().value;
    }
    ...
    try {
        ...
        const updateActivity = async (overrideState?: string) => {
             const activity: any = {
                name: config.name || '', // Default name if empty
            ...
            if (config.largeImageKey) {
                if (config.largeImageKey.startsWith('http')) {
                    try {
                        console.log('Calling fetchExternalAsset for largeImageKey:', config.largeImageKey);
                        const external = await fetchExternalAsset(client, activity.applicationId, config.largeImageKey);
                        console.log('fetchExternalAsset result:', external);
                ...
                if (config.portrait) {
                    // Portrait mode flag if supported by specific clients
                    // Removing 'mp:' prefix as it was causing the image to disappear
                    activity.assets.largeImage = activity.assets.largeImage;
                }
            }
            if (config.smallImageKey) {
                if (config.smallImageKey.startsWith('http')) {
                    try {
                        console.log('Calling fetchExternalAsset for smallImageKey:', config.smallImageKey);
                        const external = await fetchExternalAsset(client, activity.applicationId, config.smallImageKey);
                        console.log('fetchExternalAsset result:', external);
                        if (external && external.length > 0) {
