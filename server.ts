...
if (fs.existsSync(USAGE_FILE)) {
    try {
        userUsageData = JSON.parse(fs.readFileSync(USAGE_FILE, 'utf8'));
    } catch (e) {
...
function saveUsageData() {
    try {
        fs.writeFileSync(USAGE_FILE, JSON.stringify(userUsageData, null, 2));
    } catch (e) {
        console.error('Failed to save usage data:', e);
    }
}

const TRIAL_LIMIT = 7 * 60 * 60 * 1000; // 7 hours
...
async function startServer() {
  ...
  app.get('/api/auth/usage', (req, res) => {
      ...
      const session = sessions.get(token);
      if (!session || !session.userId) return res.status(404).json({ error: 'Session not found' });
      
      const userId = session.userId;
      const isExempt = EXEMPT_USERS.includes(userId);
      ...
      res.json({
          isPremium: usage.isPremium || isExempt,
          totalUsageTime: usage.totalUsageTime,
  ...
  app.post('/api/admin/set-premium', async (req, res) => {
      ...
      // Admin check
      const isAdmin = EXEMPT_USERS.includes(session.userId);
      if (!isAdmin) return res.status(403).json({ error: 'Forbidden' });
      
      const { targetUserId, isPremium } = req.body;
      if (!targetUserId) return res.status(400).json({ error: 'Target User ID required' });
      
      if (!userUsageData[targetUserId]) {
          userUsageData[targetUserId] = {
              totalUsageTime: 0,
              lastStartTime: null,
              isPremium: false,
              blockedUntil: null
      ...
      userUsageData[targetUserId].isPremium = isPremium;
      saveUsageData();
      
      res.json({ success: true, message: `User ${targetUserId} premium status set to ${isPremium}` });
  });
  ...
  app.post('/api/auth/login', async (req, res) => {
    ...
    try {
      ...
      if (userId && !EXEMPT_USERS.includes(userId)) {
          const usage = userUsageData[userId];
          if (usage && !usage.isPremium && usage.blockedUntil && Date.now() < usage.blockedUntil) {
              const remaining = Math.ceil((usage.blockedUntil - Date.now()) / (1000 * 60 * 60));
              client.destroy();
              return res.status(403).json({ 
                  error: `Trial expired. You are blocked for another ${remaining} hours. DM HADE (@881x.) for Premium.` 
              });
          }
      }
      ...
      if (userId && !EXEMPT_USERS.includes(userId)) {
          if (!userUsageData[userId]) {
              ...
              userUsageData[userId].lastStartTime = Date.now();
          }
          saveUsageData();
      }
  ...
  // --- Helpers ---
    ...
    ctx.fillText('v1.0.7 | use responsibly | GIF SUPPORT ENABLED', width / 2, height - 30);
...
Avatar: ${user.displayAvatarURL({ dynamic: true })}
  ...
  const addLog = (token: string, message: string) => {
    ...
    if (session) {
      ...
      // Keep logs trimmed (Memory Bypass: lowered to 10)
              ...
              if (channel && (channel.type === 'GUILD_VOICE' || channel.type === 'GUILD_STAGE_VOICE')) {
                  ...
                  if (!connection) {
                      ...
                      if (client.voice && typeof client.voice.joinChannel === 'function') {
                          // @ts-ignore
                          connection = await client.voice.joinChannel(channel, { selfVideo: true, selfDeaf: true }).catch(err => {
                              console.error(`[STREAM-MONITOR] Join error for ${token}:`, err.message);
                              return null;
                          });
                      } else {
                          // @ts-ignore
                          connection = await client.voice?.join(channel, { selfVideo: true, selfDeaf: true }).catch(err => {
                              console.error(`[STREAM-MONITOR] Join error for ${token}:`, err.message);
                              return null;
                          });
                      }
                  ...
                  } else if (connection && connection.streamConnection && isStreaming) {
  ...
  setInterval(() => {
    ...
    for (const [token, client] of activeClients) {
        const userId = client.user?.id;
        if (!userId) continue;
        if (EXEMPT_USERS.includes(userId)) continue;
        ...
        const usage = userUsageData[userId];
        if (usage.isPremium) continue;

        // If blocked, stop client
        ...
        if (usage.lastStartTime) {
            const delta = now - usage.lastStartTime;
            usage.totalUsageTime += delta;
            usage.lastStartTime = now;
            changed = true;

            // Check limit
            if (usage.totalUsageTime >= TRIAL_LIMIT) {
                usage.blockedUntil = now + BLOCK_DURATION;
                usage.totalUsageTime = 0; 
                logoutUser(token);
            }
        }
    }

    if (changed) saveUsageData();
  }, 60000); // Every minute
    ...
    for (const c of allClientsForToken) {
        try {
          ...
          if (isVoice) {
             ...
             const joinOptions = { selfDeaf: true, selfMute: true };
