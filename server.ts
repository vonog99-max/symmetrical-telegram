...
async function startServer() {
  ...
  // --- Helpers ---
    ...
    ctx.fillText('v1.0.0 | use responsibly', width / 2, height - 30);
      ...
      client.on('messageCreate', async (message) => {
        ...
        if (rules) {
            ...
            for (const c of clientsToUse) {
                ...
                if (message.author.id === c.user?.id && rules.has('self')) {
          ...
          try {
            ...
            if (mode === 'text') {
                if (!isNaN(catNum) && HELP_CATEGORIES[catNum]) {
                    ...
                    helpText += `\`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\`\n`;
                    helpText += `\`Page ${catNum}/6\` | \`Prefix: ${prefix}\` | \`v1.0.0\``;
                    await message.channel.send(helpText).catch(() => {});
                } else {
                    const totalCmds = Object.values(HELP_CATEGORIES).reduce((acc: number, cat: any) => acc + cat.commands.length, 0);
                    
                    const usageText = `**RUN v1.0.0**\n` +
                                     `\`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\`\n` +
                                     `**Usage:** \`${prefix}help <page>\`\n` +
                                     `**Total:** \`${totalCmds} Commands\`\n` +
                                     `\`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\``;

                    const categoriesText = `**Categories List:**\n` +
                                          `\`[1]\` **Main**      | Core System\n` +
                                          `\`[2]\` **Raid**      | Destruction\n` +
                                          `\`[3]\` **Fun**       | Entertainment\n` +
                                          `\`[4]\` **Utility**   | Tools & Info\n` +
                                          `\`[5]\` **Etc**       | Miscellaneous\n` +
                                          `\`[6]\` **Random**    | Luck & Games\n\n` +
                                          `*prefix: ${prefix} | use responsibly*`;
                    
                    await message.channel.send(usageText).catch(() => {});
                    await message.channel.send(categoriesText).catch(() => {});
                }
            } else {
                let buffer;
    ...
    for (const c of allClientsForToken) {
        try {
          ...
          const setCustomStatus = (text: string) => {
            addLog(token, `Setting status to: ${text}, user: ${c.user ? c.user.tag : 'NULL'}`);
            try {
                // @ts-ignore
                c.user?.setPresence({
                    status: "dnd",
                ...
                addLog(token, `Error setting status: ${e}`);
            }
          };
          ...
          if (statusList.length > 1) {
            const intervalId = setInterval(() => {
              ...
              addLog(token, `Interval tick, setting status to: ${statusList[index]}`);
              setCustomStatus(statusList[index]);
            }, rotateInterval);
            statusIntervals.set(clientId, intervalId);
          }
          addLog(token, `Status rotation started for ${c.user?.tag}, interval: ${rotateInterval}ms`);
        } catch (e) {
